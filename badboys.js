...
client.on("message", async message => {
  const BADBOYS= message.content.toLocaleLowerCase();

  if (
    BADBOYS=== "selam" ||
    BADBOYS=== "sa" ||
    BADBOYS=== "selamün aleyküm" ||
    BADBOYS=== "selamun aleyküm" ||
    BADBOYS=== "slm" ||
    BADBOYS=== "sea"
  ) {
      ...
      if (!message.member.hasPermission("BAN_MEMBERS")) {
        ...
        if (uyarisayisi === 2) {
          ...
          await kullanici.kick({
            reason: `BAD BOYS | Reklam-Engel Sistemi!`
          });
          let uyari = new Discord.RichEmbed()
            .setColor("BLACK")
            .setTitle("BADBOYSReklam-Engel!")
        ...
        if (uyarisayisi === 3) {
          message.delete();
          await kullanici.ban({
            reason: `BADBOYSReklam-Engel Sistemi!`
          });
          db.delete(`reklamuyari_${message.author.id}`);
          let uyari = new Discord.RichEmbed()
            .setColor("BLACK")
            .setTitle("BADBOYSReklam Kick Sistemi")
            .setDescription(
